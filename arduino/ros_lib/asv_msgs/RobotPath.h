...
namespace asv_msgs
{
  ...
  class RobotPath : public ros::Msg
  {
    public:
      ...
      typedef geometry_msgs::Pose _list_type;
      _list_type st_list;
      _list_type * list;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(list_lengthT > list_length)
        this->list = (geometry_msgs::Pose*)realloc(this->list, list_lengthT * sizeof(geometry_msgs::Pose));
      list_length = list_lengthT;
      for( uint32_t i = 0; i < list_length; i++){
      offset += this->st_list.deserialize(inbuffer + offset);
        memcpy( &(this->list[i]), &(this->st_list), sizeof(geometry_msgs::Pose));
      }
    ...
    const char * getMD5(){ return "26644de2debc53d8bba799cb1ae600b6"; };
