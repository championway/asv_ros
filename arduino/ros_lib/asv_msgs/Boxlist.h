#ifndef _ROS_asv_msgs_Boxlist_h
#define _ROS_asv_msgs_Boxlist_h
...
namespace asv_msgs
{
  ...
  class Boxlist : public ros::Msg
  {
    public:
      ...
      typedef asv_msgs::Box _list_type;
      _list_type st_list;
      _list_type * list;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(list_lengthT > list_length)
        this->list = (asv_msgs::Box*)realloc(this->list, list_lengthT * sizeof(asv_msgs::Box));
      list_length = list_lengthT;
      for( uint32_t i = 0; i < list_length; i++){
      offset += this->st_list.deserialize(inbuffer + offset);
        memcpy( &(this->list[i]), &(this->st_list), sizeof(asv_msgs::Box));
      }
      union {
        int32_t real;
        uint32_t base;
    ...
    const char * getType(){ return "asv_msgs/Boxlist"; };
    const char * getMD5(){ return "64ed701dbabc3b78cf16142116cf4f2f"; };
