   ...
   public:
     ...
     */
     void initial(Node start, Node end, Eigen::MatrixXd map){
       ...
       end_ = end;
       map_ = map;  
       /*int map_size = int(21/2)*2;  //set maze size
       for(int x=-map_size; x<map_size; x++){
         map = 
       }*/


       // Set the maze around the start point to 0
       for(int x=-1; x<=1; ++x){
        for(int y=-1; y<=1; ++y){
          map_(start.get_x()+x, start.get_y()+y) = 0;
        }
       }
       height = map.rows(); width = map.cols();
       if(!checkIfValid()) {
       ...
       while(open.size()!=0){
         ...
         *current_node = open.front();
         ...
         if(*candidatePtr==end_){
           ...
           clear_list();
           if(ASTAR_SHOWTIME) printf("[AStar] Cost time: %f seconds\n", double((clock_t()-start_t)/CLOCKS_PER_SEC));
           return true;
         } // End if
         // --------------------------------------------- Planning ---------------------------------------------
         // Generate children
         ...
         for(nit=children.begin(); nit!=children.end(); ++nit){
           ...
           //std::cout<< "g:" << nit->get_g() << ", h:" << nit->get_h() << ", map:" << map_(nit->get_x(), nit->get_y()) << std::endl;
           nit->set_f(nit->get_g() + nit->get_h() + 1.8*map_(nit->get_x(), nit->get_y()));
